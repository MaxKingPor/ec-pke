...
use ec_pke::{Cipher, EcDecrypt, EcEncrypt, Mode};
use rand_core::OsRng;
use sm2::{Scalar, SecretKey, Sm2, U256};
...
fn decrypt_verify() {
    let public_key: SecretKey = NonZeroScalar::<Sm2>::try_from(PRIVATE_KEY.as_ref() as &[u8])
        .unwrap()
        .into();
    let cipher = Cipher::from_slice(&CIPHER, Mode::C1C3C2).unwrap();
    let mut out = vec![0; cipher.c2().len()];
    public_key
        .decrypt_digest_into::<sm3::Sm3>(&cipher, &mut out)
        .unwrap();
    assert_eq!(out, MSG);
}
...
fn decrypt_der_verify() {
    let dk: SecretKey = NonZeroScalar::<Sm2>::try_from(PRIVATE_KEY.as_ref() as &[u8])
        .unwrap()
        .into();
    let cipher = Cipher::from_der(&ASN1_CIPHER).unwrap();
    let mut out = vec![0; cipher.c2().len()];
    dk.decrypt_digest_into::<sm3::Sm3>(&cipher, &mut out)
        .unwrap();
    assert_eq!(out, MSG);
}
